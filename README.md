...
"system-images;android-36.1;google_apis_playstore;x86_64"`, fdroidserver, Brave Browser, Proton Mail, Proton Mail Bridge, Bottles, Ente Auth, Discord, OBS Studio, HandBrake, FreeTube, GIMP, Luanti, Telegram, Pied (run `flatpak run com.mikeasoft.pied` to setup it), Aisleriot Solitaire, Kdenlive, KiCad (can be launched with desktop entry `~/.local/share/applications/kicad.desktop`), ClipCascade (server enabled with `systemctl --user enable --now clipcascade-server` on 
