...
<li>To connect to a captive portal such as a login page of a public Wi-Fi with the cost of DNS leak, run `dns_down` and turn off Trusted Recursive Resolver (TRR) / DNS-over-HTTPS (DoH) only mode in your browser if enabled, for Firefox, that is, Privacy and Security > DNS over HTTPS > Custom: Always use secure DNS with control over your provider and fallback behavior or `user_pref("network.trr.mode", 3);`, and run `dns_up` after connected.<li>
<li>Run
