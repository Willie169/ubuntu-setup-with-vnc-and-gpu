...
`http://localhost:8082`, client enabled with `systemctl --user enable --now clipcascade-client`), Phice (can be started by running `cd ~/phice && uv run gunicorn -b 127.0.0.1:<port> -w 4 "app:app"`, or `phice [port]`, where if port is not specified, `5001` is used, and accessed on `localhost:<port>`), Krita, MuseScore, OnlyOffice, VLC, 
