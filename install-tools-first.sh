...
test -f /etc/xdg/autostart/gnome-software-service.desktop && sudo mv /etc/xdg/autostart/gnome-software-service.desktop /etc/xdg/autostart/gnome-software-service.desktop.bak || true
...
sudo DEBIAN_FRONTEND=noninteractive apt install apt-transport-https bash build-essential ca-certificates coreutils cmake curl dbus openjdk-21-jdk g++ gcc git gnupg grep gzip jq locales lsb-release make ninja-build octave octave-dev openssh-server perl perl-tk python-is-python3 python3 vim-gtk3 wget xz-utils -y -o Dpkg::Options::="--force-confnew" -o Dpkg::Options::="--force-overwrite"
sudo DEBIAN_FRONTEND=noninteractive apt install apparmor-utils clinfo dnscrypt-proxy fcitx5 fcitx5-configtool fcitx5-frontend-all fcitx5-rime flatpak keyd kiwix language-pack-gnome-en pipewire pipewire-audio-client-libraries snapd ufw unattended-upgrades wireplumber -y -o Dpkg::Options::="--force-confnew" -o Dpkg::Options::="--force-overwrite"
...
octave -G -e 'pkg install -forge control'
...
ExecStart=/bin/python3 generate-domains-blocklist.py -o blocklist.txt -a domains-allowlist.txt ; sleep 2 ; systemctl restart dnscrypt-proxy.service
