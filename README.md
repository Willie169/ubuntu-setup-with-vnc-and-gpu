...
Configures dual-boot friendly configurations, remaps Copilot key (`f23+leftshift+leftmeta`) to right Ctrl key (`rightcontrol`), enables automatic login if on GDM, LightDM, KDE Plasma, or LXQt, enables Wayland if on GDM or KDE Plasma, installs recommended drivers and tools for C, C++, Python3, Java21, Node.js LTS (via NVM), Rust, Perl, Aptitude, GitHub CLI, GitLab CLI, Kate, GVim, OpenSSL, OpenSSH, Tailscale, jq, mikefarah's yq, FFMPEG, Fcitx5, 
