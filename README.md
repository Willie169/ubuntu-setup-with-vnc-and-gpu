...
Android SDK Command-line tools, emulator, and platform-tools, fdroidserver, Brave Browser, Bottles, Ente Auth, OBS Studio, HandBrake, FreeTube, GIMP, Luanti, Telegram, Parabolic, Pied (run `flatpak run com.mikeasoft.pied` to setup it), Kdenlive, KiCad (can be launched with desktop entry `~/.local/share/applications/kicad.desktop`), ClipCascade (server enabled with `systemctl --user enable --now clipcascade-server` on 
