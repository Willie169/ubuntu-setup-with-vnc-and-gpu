...
Android Studio, Android CLI, Android Command-line tools, Android SDK `"emulator"` and `"platform-tools"`, 
fdroidserver, Brave Browser, Bottles, Ente Auth, OBS Studio, HandBrake, FreeTube, GIMP, Luanti, Telegram, Pied (run `flatpak run com.mikeasoft.pied` to setup it), Kdenlive, KiCad (can be launched with desktop entry `~/.local/share/applications/kicad.desktop`), ClipCascade (server enabled with `systemctl --user enable --now clipcascade-server` on 
