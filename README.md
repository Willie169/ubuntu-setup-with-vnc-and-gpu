...
Configures dual-boot friendly configurations, enables automatic login if on GDM, SDDM, or LightDM, enables Wayland if on GDM or KDE Plasma, disables KDE Wallet if installed, installs recommended drivers and tools for C, C++, Python3, Java21, Node.js LTS (via NVM), Rust, Perl, Aptitude, GitHub CLI, GitLab CLI, Kate, GVim, OpenSSL, OpenSSH, Tailscale, GNOME Keyring, jq, mikefarah's yq, FFMPEG, Fcitx5, 
