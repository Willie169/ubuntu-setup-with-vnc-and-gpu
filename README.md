...
in `.bashrc`), ANTLR 4 (JAR in `/usr/local/java`), Tor, Tor Browser, PlantUML (JAR in `/usr/local/java`), clang-uml, SQLite 3, stress-ng, iotop-c, ntopng, PostgreSQL, LocalSend, RustDesk (check Enable direct IP access under Security to use it with Tailscale), RustDesk server (in client, in ID/Relay server, paste the IP of the machine in ID server and Relay server and the public key in `/var/lib/rustdesk-server/id_ed25519.pub` in Key), 
Android SDK Command-line tools, emulator, and platform-tools, fdroidserver, gallery-dl, Brave Browser, Bottles, Ente Auth, OBS Studio, HandBrake, FreeTube, GIMP, Luanti, vokoscreenNG, Telegram, Parabolic, Pied (run `flatpak run com.mikeasoft.pied` to setup it), Kdenlive, KiCad (can be launched with desktop entry `~/.local/share/applications/kicad.desktop`), ClipCascade (server enabled with `systemctl --user enable --now clipcascade-server` on 
