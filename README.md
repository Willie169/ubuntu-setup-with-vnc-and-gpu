...
lftp, LibreOffice, LibreTranslate (server enabled with `systemctl --user enable --now libretranslate` on `http://localhost:5000`), fdupes, bnkextr, vgmstream, Hugo, gh2md, img2pdf, English, Chinese - Simplified, Chinese - Simplified (vertical), Chinese - Traditional, and Chinese - Traditional (vertical) Tesseract OCR, OCRmyPDF, Jupyter Notebook, JupyterLab, Jupytext, Meson, pylatexenc, The Unarchiver (unar), RARLAB UnRAR, Verilator, Ngspice, jpegoptim, optipng, exiftool, libavif, libjxl, libheif, LibWebP, ImageMagick, Inkscape, CUPS, SANE, Document Scanner, 
