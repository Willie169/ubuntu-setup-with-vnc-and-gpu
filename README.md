...
AppImageLauncher, Kiwix, yosys – Yosys Open SYnthesis Suite, Element Desktop (can be launched by running `element-desktop` or with desktop entry), Newsflash, JamesDSP, Octave, VSCodium, qBittorrent, Remmina, Tor, Tor Browser, PlantUML, clang-uml, SQLite 3, mpv, iotop-c, netcat, NetHogs, net-tools, iftop, LocalSend, RustDesk, Android SDK Command-line tools and platform-tools, Better ADB Sync, fdroidserver, gallery-dl, Brave Browser, Bottles, Ente Auth, OBS Studio, HandBrake, GIMP, Luanti (Flatpak, data in `~/.var/app/org.luanti.luanti/.minetest`, which is exported as `$MINETEST` in `~/.bashrc` and can be launched with NVIDIA GPU if available by running `luanti`), vokoscreenNG, Pied (can be updated with `update_pied`), FreeCAD, Kdenlive, Phice (can be started by running `cd ~/phice && uv run gunicorn -b 127.0.0.1:<port> -w 4 "app:app"`, or `phice [port]`, where if port is not specified, `5001` is used, and accessed on `localhost:<port>`), scrcpy, Krita, MuseScore, OnlyOffice, VLC, 
