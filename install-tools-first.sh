...
PKG='apparmor-utils aria2 bridge-utils clang-uml clinfo dnscrypt-proxy fcitx5 fcitx5-configtool fcitx5-frontend-all fcitx5-rime filelight flatpak fwupd gtkwave kate krita language-pack-gnome-en libfuse2t64 libvirt-daemon-system libvirt-clients lxc lxc-templates ntfs-3g obs-studio opencc ovmf pipewire pipewire-audio-client-libraries qalculate-gtk qemu-system-gui qemu-system-x86 qemu-user-binfmt qemu-user qemu-utils qtspeech5-speechd-plugin quickemu remmina remmina-plugin-rdp remmina-plugin-secret snapd spice-vdagent swtpm swtpm-tools testdisk torbrowser-launcher transgui ufw uidmap unattended-upgrades virt-manager virt-viewer wireplumber wl-clipboard xclip'
# shellcheck disable=2086
if [ "$TEST" -eq 0 ]; then
  sudo DEBIAN_FRONTEND=noninteractive apt install $PKG -y -o Dpkg::Options::="--force-confnew" -o Dpkg::Options::="--force-overwrite"
else
